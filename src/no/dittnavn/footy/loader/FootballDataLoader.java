    ...
    public static List<Match> load(String filePath, String league) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                m.setHomeTeam(
                        get(row, index, "HomeTeam").isEmpty()
                                ? get(row, index, "Home")
                                : get(row, index, "HomeTeam")
                );

                m.setAwayTeam(
                        get(row, index, "AwayTeam").isEmpty()
                                ? get(row, index, "Away")
                                : get(row, index, "AwayTeam")
                );
                ...
                // goals
                m.setHomeGoals(parseInt(
                        get(row, index, "FTHG").isEmpty()
                                ? get(row, index, "HG")
                                : get(row, index, "FTHG")
                ));

                m.setAwayGoals(parseInt(
                        get(row, index, "FTAG").isEmpty()
                                ? get(row, index, "AG")
                                : get(row, index, "FTAG")
                ));
                ...
                // shots on target
                m.setHomeShotsTarget(parseInt(
                        get(row, index, "HST").isEmpty()
                                ? get(row, index, "homeShotsTarget")
                                : get(row, index, "HST")
                ));

                m.setAwayShotsTarget(parseInt(
                        get(row, index, "AST").isEmpty()
                                ? get(row, index, "awayShotsTarget")
                                : get(row, index, "AST")
                ));
                ...
                // Bet365 odds
                m.setHomeOdds(parseDouble(
                        get(row, index, "B365H").isEmpty()
                                ? get(row, index, "B365CH")
                                : get(row, index, "B365H")
                ));

                m.setDrawOdds(parseDouble(
                        get(row, index, "B365D").isEmpty()
                                ? get(row, index, "B365CD")
                                : get(row, index, "B365D")
                ));

                m.setAwayOdds(parseDouble(
                        get(row, index, "B365A").isEmpty()
                                ? get(row, index, "B365CA")
                                : get(row, index, "B365A")
                ));
